#!/bin/bash
if [ `ps -e | grep -c bot.sh` -gt 2 ]; then echo "Already running, i'm killing old process, please run it again!"; killall -9 bot.sh; killall -9 Xvfb; killall -9 chrome; killall -9 chromium; killall -9 chromium; killall -9 sleep && exit 1; fi
...
do
    ...
    do
        ...
        DISPLAY=:1.1 chromium --no-sandbox --new-window --user-data-dir="/root/chromeBotTE" --disable-popup-blocking --incognito ${links[$i]} & disown
        chromePIDs[$i]=$!
    done
    sleep ${timer}
